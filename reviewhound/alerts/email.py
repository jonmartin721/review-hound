...
def format_review_alert(
    business_name: str, source: str, rating: float, sentiment_label: str, text: str, author: str
) -> tuple[str, str]:
        ...
        <div style="padding: 20px; background: #f9fafb;">
            ...
            <div style="background: white; border-radius: 8px; padding: 16px; margin: 16px 0;
                ...
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
                    ...
                    <span style="background: {sentiment_color}; color: white; padding: 2px 8px;
                        border-radius: 4px;">{sentiment_label.title()}</span>
                </div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
...
def check_and_send_alerts(session, business, review) -> int:
    ...
    configs = (
        ...
        .filter(AlertConfig.business_id == business.id, AlertConfig.enabled.is_(True))
        .all()
    )
    ...
    for config in configs:
        ...
        if config.alert_on_negative and review.rating and review.rating <= config.negative_threshold:
            should_alert = True
